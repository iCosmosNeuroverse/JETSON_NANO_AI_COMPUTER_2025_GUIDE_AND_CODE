#Modified by Cosmos Bennett to utilize IpCamera/Android etc
...
Usage:
    manage.py (drive) [--model=<model>] [--js] [--type=(linear|categorical)] [--camera=(single|stereo)] [--meta=<key:value> ...] [--myconfig=<filename>] [--ip_cam_url=<ip cam url>]
...
Options:
    ...
    --myconfig=filename     Specify myconfig file to use. 
                            [default: myconfig.py]
    --ip_cam_url            Ip Cam url of camera device, android eg, with app iPwebcam and server started
"""
...
class PilotDebug:
    def run(self, angle, throttle, image):
        print(f"[PilotDebug] angle={angle:.3f}, throttle={throttle:.3f}")
        img = image.copy()
        cv2.putText(img, f"A:{angle:.2f} T:{throttle:.2f}", (10,20),
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    #Begin CosmosModification
    print("Heating up your camera...")
    while True:
        img = V.mem.get('image')
        if img is not None:
...
def add_camera(V, cfg, camera_type):
	...
	#CosmosModification--adds IPCamera android etc instead of default cam types shipped with donkey car
        V.add(IPCamera(ip_cam_url, 160, 120 ), inputs=inputs, outputs=outputs, threaded=False) 
...
if __name__ == '__main__':
    ...
    if args['drive']:
        ...
        drive(cfg, model_path=args['--model'], use_joystick=args['--js'],
              ...
              meta=args['--meta'],ip_cam_url=args['--ip_cam_url'])
    elif args['train']:
